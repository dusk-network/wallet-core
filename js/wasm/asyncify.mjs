...
class Asyncify {
  ...
  wrapExports(exports, exportsToWrap) {
    ...
    for (let exportName in exports) {
      ...
      if (
        ...
        exportsToWrap.includes(exportName)
      ) {
  ...
  init(instance, imports, exportsToWrap) {
    ...
    this.exports = this.wrapExports(exports, exportsToWrap);
...
export async function instantiate(source, imports, exportsToWrap) {
  let state = new Asyncify();
...
export async function instantiateStreaming(source, imports, exportsToWrap) {
  let state = new Asyncify();
  let result = await WebAssembly.instantiateStreaming(
    source,
    state.wrapImports(imports)
  );
  state.init(result.instance, imports, exportsToWrap);
  return result;
}
